...
void main() {
  test('DefaultShaderWarmUp has expected canvas invocations', () {
    ...
    for (int i = 0; i < canvas.invocations.length - 1; i += 1) {
      if (canvas.invocations[i].memberName == #clipRRect &&
          canvas.invocations[i + 1].memberName == #drawRect) {
  ...
  test('ShaderWarmUp.execute disposes the image and picture', () async {
    ...
    expect(
      ...
      throwsA(isA<String>().having(
          (String message) => message, 'message', 'Object has been disposed.')),
    );
    expect(capturedImage.debugDisposed, true);
  },
      skip:
          kIsWeb); // [intended] Browser doesn't support approximateBytesUsed and doesn't rasterize the picture at this time.
}
