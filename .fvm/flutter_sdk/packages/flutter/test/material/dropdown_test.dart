...
class _TestAppState extends State<TestApp> {
  ...
  Widget build(BuildContext context) {
    return Localizations(
      ...
      child: MediaQuery(
        data:
            MediaQueryData.fromWindow(window).copyWith(size: widget.mediaSize),
...
void main() {
  ...
  testWidgets('disabledHint behavior test', (WidgetTester tester) async {
    ...
    await tester
        .pumpWidget(build(items: menuItems, onChanged: null, value: 'two'));
  ...
  testWidgets('DropdownButtonFormField can be focused, and has focusColor',
    ...
    final Finder buttonFinder = find.descendant(
        of: find.byKey(buttonKey), matching: find.byType(InputDecorator));
    expect(
        buttonFinder,
        paints
          ..rrect(
              rrect: const RRect.fromLTRBXY(0.0, 12.0, 800.0, 60.0, 4.0, 4.0),
              color: const Color(0x1f000000)));
  ...
  testWidgets('borderRadius property works properly',
    ...
    final InkWell firstItem =
        tester.widget(find.widgetWithText(InkWell, 'One'));
    final InkWell lastItem =
        tester.widget(find.widgetWithText(InkWell, 'Four'));

    expect(firstItem.borderRadius,
        const BorderRadius.vertical(top: Radius.circular(radius)));
    expect(lastItem.borderRadius,
