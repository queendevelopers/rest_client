...
class Daemon {
  ...
  final Map<String, Completer<dynamic>> _outgoingRequestCompleters =
  ...
  void _handleRequest(Map<String, dynamic> request) {
    ...
    try {
      ...
      if (method != null) {
        ...
        _domainMap[prefix].handleCommand(name, id,
            castStringKeyedMap(request['params']) ?? const <String, dynamic>{});
      } else {
        // If there was no 'method' field then it's a response to a daemon-to-editor request.
        final Completer<dynamic> completer =
            _outgoingRequestCompleters[id.toString()];
        if (completer == null) {
...
class DaemonDomain extends Domain {
  ...
  Future<String> exposeUrl(String url) async {
    final dynamic res =
        await daemon.sendRequest('app.exposeUrl', <String, String>{'url': url});
...
void stdoutCommandResponse(Map<String, dynamic> command) {
  globals.stdio.stdoutWrite(
    ...
    fallback: (String message, dynamic error, StackTrace stack) {
      throwToolExit(
          'Failed to write daemon command response to stdout: $error');
    },
  );
}
