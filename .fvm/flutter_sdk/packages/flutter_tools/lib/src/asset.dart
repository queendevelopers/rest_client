  ...
  Future<int> build({
    ...
    if (deferredComponentsAssetVariants != null) {
      for (final String componentName in deferredComponentsAssetVariants.keys) {
        ...
        for (final _Asset asset
            in deferredComponentsAssetVariants[componentName].keys) {
          final File assetFile = asset.lookupAssetFile(_fileSystem);
          if (!assetFile.existsSync() &&
              deferredComponentsAssetVariants[componentName][asset].isEmpty) {
          ...
          // variant is preferred.
          if (assetFile.existsSync()) {
            assert(!deferredComponentsAssetVariants[componentName][asset]
                .contains(asset));
            deferredComponentsAssetVariants[componentName][asset]
                .insert(0, asset);
          }
          for (final _Asset variant
              in deferredComponentsAssetVariants[componentName][asset]) {
            final File variantFile = variant.lookupAssetFile(_fileSystem);
            assert(variantFile.existsSync());
  ...
  List<_Asset> _getMaterialAssets() {
    ...
    for (final Map<String, Object> family in kMaterialFonts) {
      for (final Map<String, Object> font
          in family['fonts'] as List<Map<String, Object>>) {
  ...
  DevFSStringContent _createAssetManifest(
    ...
    for (final _Asset main in assets) {
      jsonEntries[main] = <String>[
        for (final _Asset variant in assetVariants[main]) variant.entryUri.path,
      ];
    }
    if (deferredComponentsAssetVariants != null) {
