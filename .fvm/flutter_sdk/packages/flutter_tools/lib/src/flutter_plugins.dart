...
void _renderTemplateToFile(String template, Object? context, File file,
  ...
  final String renderedTemplate =
      templateRenderer.renderString(template, context, htmlEscapeValues: false);
    ...
    case AndroidEmbeddingVersion.v2:
      ...
      for (final Map<String, Object?> plugin in androidPlugins) {
        final bool supportsEmbeddingV1 =
            (plugin['supportsEmbeddingV1'] as bool?) == true;
        final bool supportsEmbeddingV2 =
            (plugin['supportsEmbeddingV2'] as bool?) == true;
...
Future<void> _writeIOSPluginRegistrant(
  ...
  final List<Map<String, Object?>> iosPlugins =
      _extractPlatformMaps(plugins, IOSPlugin.kConfigKey);
...
Future<void> _writeLinuxPluginFiles(
    FlutterProject project, List<Plugin> plugins) async {
  final List<Plugin> nativePlugins =
      _filterNativePlugins(plugins, LinuxPlugin.kConfigKey);
  final List<Map<String, Object?>> linuxPlugins =
      _extractPlatformMaps(nativePlugins, LinuxPlugin.kConfigKey);
  final Map<String, Object> context = <String, Object>{
    'os': 'linux',
...
Future<void> _writeMacOSPluginRegistrant(
    FlutterProject project, List<Plugin> plugins) async {
  final List<Plugin> nativePlugins =
      _filterNativePlugins(plugins, MacOSPlugin.kConfigKey);
  final List<Map<String, Object?>> macosPlugins =
      _extractPlatformMaps(nativePlugins, MacOSPlugin.kConfigKey);
  final Map<String, Object> context = <String, Object>{
    'os': 'macos',
...
Future<void> writeWindowsPluginFiles(FlutterProject project,
  ...
  final List<Plugin> nativePlugins =
      _filterNativePlugins(plugins, WindowsPlugin.kConfigKey);
  final List<Plugin> win32Plugins = _filterPluginsByVariant(
      nativePlugins, WindowsPlugin.kConfigKey, PluginPlatformVariant.win32);
  final List<Map<String, Object?>> pluginInfo =
...
Future<void> writeWindowsUwpPluginFiles(FlutterProject project,
  ...
  final List<Plugin> nativePlugins =
      _filterNativePlugins(plugins, WindowsPlugin.kConfigKey);
  final List<Plugin> uwpPlugins = _filterPluginsByVariant(
      nativePlugins, WindowsPlugin.kConfigKey, PluginPlatformVariant.winuwp);
  final List<Map<String, Object?>> pluginInfo =
