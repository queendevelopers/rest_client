...
class AndroidDevice extends Device {
  ...
  Future<bool> _checkForSupportedAdbVersion() async {
    ...
    try {
      ...
      _logger.printError(
          'The ADB at "${_androidSdk.adbPath}" is too old; please install version 1.0.39 or later.');
  ...
  Future<bool> _installApp(
    ...
    if (!app.file.existsSync()) {
      _logger.printError(
          '"${_fileSystem.path.relative(app.file.path)}" does not exist.');
      return false;
    }
  ...
  Future<bool> _isAdbValid() async {
    return _adbIsValid ??= await _checkForSupportedAdbVersion() &&
  ...
  DevicePortForwarder get portForwarder =>
      _portForwarder ??= AndroidDevicePortForwarder(
