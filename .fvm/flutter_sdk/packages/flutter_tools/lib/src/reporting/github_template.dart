...
class GitHubTemplateCreator {
  ...
  Future<String> toolCrashIssueTemplateGitHubURL(String command, dynamic error,
  ...
  String _projectMetadataInformation() {
    ...
    try {
      ...
      final StringBuffer description = StringBuffer()
        ..writeln(
            '**Type**: ${flutterProjectTypeToString(metadata.projectType)}')
  ...
  Future<String> _shortURL(String fullURL) async {
    ...
    try {
      ...
      final List<int> bodyBytes =
          utf8.encode('url=${Uri.encodeQueryComponent(fullURL)}');
      final HttpClientRequest request =
          await _client.postUrl(Uri.parse('https://git.io'));
      request.headers
      ...
      if (response.statusCode == 201) {
        ...
        _logger.printTrace(
            'Failed to shorten GitHub template URL. Server responded with HTTP status code ${response.statusCode}');
      }
