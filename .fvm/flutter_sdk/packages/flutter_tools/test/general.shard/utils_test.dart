...
''');
  ...
  group('text wrapping', () {
    ...
    final String _longLineWithNewlines =
        'This is a long line with newlines that\n'
        'needs to be wrapped.\n\n'
        '${'0123456789' * 5}';
    final String _longAnsiLineWithNewlines =
        ...
        '${'0123456789' * 3}'
        '${AnsiTerminal.green}0123456789${AnsiTerminal.resetAll}';
    const String _onlyAnsiSequences =
    ...
    const String _indentedLongLine =
    ...
    testWithoutContext('does not wrap by default in tests', () {
      expect(wrapText(_longLine, columnWidth: 80, shouldWrap: true),
          equals(_longLine));
    });

    testWithoutContext('can override wrap preference if preference is off', () {
      expect(wrapText(_longLine, columnWidth: _lineLength, shouldWrap: true),
          equals('''
...
wrapped.'''));
    ...
    testWithoutContext('can override wrap preference if preference is on', () {
      expect(wrapText(_longLine, shouldWrap: false, columnWidth: 80),
          equals(_longLine));
    });

    testWithoutContext('does not wrap at all if not told to wrap', () {
      expect(wrapText(_longLine, columnWidth: 80, shouldWrap: false),
          equals(_longLine));
    });

    testWithoutContext('does not wrap short lines.', () {
      expect(wrapText(_shortLine, columnWidth: _lineLength, shouldWrap: true),
          equals(_shortLine));
    });
...
wrapped.'''));
    ...
    testWithoutContext('wrap long lines with no whitespace', () {
      expect(
          wrapText('0123456789' * 5,
              columnWidth: _lineLength, shouldWrap: true),
          equals('''
...
0123456789'''));
    ...
    testWithoutContext('refuses to wrap to a column smaller than 10 characters',
        () {
      expect(
          wrapText('$_longLine ${'0123456789' * 4}',
...
0123456789'''));
    ...
    testWithoutContext('wraps text with newlines', () {
      expect(
          wrapText(_longLineWithNewlines,
              columnWidth: _lineLength, shouldWrap: true),
          equals('''
This is a long line with newlines that
needs to be wrapped.
...
0123456789'''));
    ...
    testWithoutContext('wraps text with ANSI sequences embedded', () {
      expect(
          wrapText(_longAnsiLineWithNewlines,
              columnWidth: _lineLength, shouldWrap: true),
          equals('''
${AnsiTerminal.red}This${AnsiTerminal.resetAll} is a long line with newlines that
needs to be wrapped.
...
wrapped.'''));
    ...
    testWithoutContext('honors hangingIndent parameter', () {
      expect(
          wrapText(_longLine,
              columnWidth: _lineLength, hangingIndent: 6, shouldWrap: true),
          equals('''
This is a long line that needs to be
      wrapped.'''));
    });
...
'''));
    ...
    testWithoutContext('honors indent parameter on already indented line.', () {
      expect(
          wrapText(_indentedLongLine,
              columnWidth: _lineLength, indent: 6, shouldWrap: true),
          equals('''
          This is an indented long line
