  ...
  group('fuchsia device', () {
    ...
    testWithoutContext('can parse ffx output for single device', () async {
      final FakeFuchsiaWorkflow fuchsiaWorkflow = FakeFuchsiaWorkflow(
          shouldUseDeviceFinder: false, canListDevices: true);
    ...
    testWithoutContext('can parse device-finder output for single device',
        () async {
      final FakeFuchsiaWorkflow fuchsiaWorkflow = FakeFuchsiaWorkflow(
          shouldUseDeviceFinder: true, canListDevices: true);
      final FakeFuchsiaSdk fuchsiaSdk = FakeFuchsiaSdk(
          devFinderDevices:
              '2001:0db8:85a3:0000:0000:8a2e:0370:7334 paper-pulp-bush-angel');
      final FuchsiaDevices fuchsiaDevices = FuchsiaDevices(
        platform: FakePlatform(
          operatingSystem: 'linux',
          environment: <String, String>{'FUCHSIA_DISABLED_ffx_discovery': '1'},
        ),
        fuchsiaSdk: fuchsiaSdk,
        fuchsiaWorkflow: fuchsiaWorkflow,
        logger: BufferLogger.test(),
    ...
    testWithoutContext('can parse device-finder output for multiple devices',
        () async {
      final FakeFuchsiaWorkflow fuchsiaWorkflow = FakeFuchsiaWorkflow(
          shouldUseDeviceFinder: true, canListDevices: true);
      final FakeFuchsiaSdk fuchsiaSdk = FakeFuchsiaSdk(
    ...
    testWithoutContext('can parse junk output from ffx', () async {
      final FakeFuchsiaWorkflow fuchsiaWorkflow = FakeFuchsiaWorkflow(
          shouldUseDeviceFinder: false, canListDevices: false);
      final FakeFuchsiaSdk fuchsiaSdk =
          FakeFuchsiaSdk(devices: 'junk', devFinderDevices: 'junk');
      final FuchsiaDevices fuchsiaDevices = FuchsiaDevices(
        platform: FakePlatform(operatingSystem: 'linux'),
        fuchsiaSdk: fuchsiaSdk,
        fuchsiaWorkflow: fuchsiaWorkflow,
...
[2018-11-09 01:30:12][52580][52983][log] INFO: example_app.cmx(flutter): Did thing this time
  ...
  ''';
      ...
      setUp(() {
        ...
        devFinder = memoryFileSystem.file('device-finder')
          ..writeAsStringSync('\n');
...
class FakeFuchsiaSdk extends Fake implements FuchsiaSdk {
  ...
  Future<String> listDevices(
      {Duration timeout, bool useDeviceFinder = false}) async {
    if (useDeviceFinder) {
...
class FakeDartDevelopmentService extends Fake
  ...
  Future<void> startDartDevelopmentService(
      Uri observatoryUri, int hostPort, bool ipv6, bool disableServiceAuthCodes,
      {Logger logger}) async {}
