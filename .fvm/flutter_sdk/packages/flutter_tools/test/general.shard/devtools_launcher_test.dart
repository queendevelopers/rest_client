...
void main() {
  ...
  setUp(() {
    ...
    final Directory tempDir = globals.fs.systemTempDirectory
        .createTempSync('flutter_devtools_launcher_test.');
  ...
  testWithoutContext(
      'DevtoolsLauncher does not launch devtools if unable to reach pub.dev and there is no activated package',
      () async {
  ...
  testWithoutContext(
    ...
    final DevtoolsLauncher launcher = DevtoolsServerLauncher(
      ...
      platform: FakePlatform(
          environment: <String, String>{'PUB_HOSTED_URL': 'https://pub2.dev'}),
      persistentToolState: persistentToolState,
      httpClient: FakeHttpClient.list(<FakeRequest>[
        FakeRequest(
          Uri.https('pub2.dev', ''),
  ...
  testWithoutContext('DevtoolsLauncher handles an invalid PUB_HOSTED_URL',
      () async {
    final DevtoolsLauncher launcher = DevtoolsServerLauncher(
      pubExecutable: 'pub',
      fileSystem: fakefs,
      logger: logger,
      platform: FakePlatform(
          environment: <String, String>{'PUB_HOSTED_URL': r'not_an_http_url'}),
      persistentToolState: persistentToolState,
      httpClient: FakeHttpClient.list(<FakeRequest>[]),
      processManager: FakeProcessManager.list(<FakeCommand>[
        const FakeCommand(
    ...
    final DevtoolsLauncher launcher = DevtoolsServerLauncher(
      ...
      httpClient: FakeHttpClient.list(<FakeRequest>[
        FakeRequest(
          Uri.https('pub.dev', ''),
          method: HttpMethod.head,
          response: const FakeResponse(statusCode: HttpStatus.forbidden),
        ),
      ]),
      processManager: FakeProcessManager.list(<FakeCommand>[
        const FakeCommand(
