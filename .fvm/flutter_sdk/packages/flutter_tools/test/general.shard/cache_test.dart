...
void main() {
  ...
  group('Cache.checkLockAcquired', () {
    ...
    testWithoutContext('should not throw when lock is acquired', () async {
      ...
      try {
        ...
        skip:
            true); // TODO(jonahwilliams): implement support for lock so this can be tested with the memory file system.
  ...
  testWithoutContext(
      ...
      () async {
    final FileSystem fileSystem = MemoryFileSystem.test();
    final Cache cache = FakeSecondaryCache()..version = '123456';
  ...
  testWithoutContext('FlutterWebSdk deletes previous directory contents', () {
    ...
    final FlutterWebSdk webSdk =
        FlutterWebSdk(cache, platform: FakePlatform(operatingSystem: 'linux'));
...
''');
    ...
    fileSystem
        .file('.pub-cache/hosted/pub.dartlang.org/example-7.0.0/lib/foo.dart')
        .createSync(recursive: true);
...
class FakeDownloadedArtifact extends CachedArtifact {
  FakeDownloadedArtifact(this.downloadedFile, Cache cache)
      : super(
          'fake',
          cache,
          DevelopmentArtifact.universal,
