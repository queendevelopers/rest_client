...
void main() {
  ...
  group('daemon', () {
    ...
    testUsingContext('daemon.version command should succeed', () async {
      final StreamController<Map<String, dynamic>> commands =
          StreamController<Map<String, dynamic>>();
      final StreamController<Map<String, dynamic>> responses =
          StreamController<Map<String, dynamic>>();
      daemon = Daemon(
        commands.stream,
        responses.add,
      ...
      final Map<String, dynamic> response =
          await responses.stream.firstWhere(_notEvent);
    ...
    testUsingContext('daemon.getSupportedPlatforms command should succeed',
      ...
      expect(response['id'], 0);
      expect(response['result'], isNotEmpty);
      expect((response['result'] as Map<String, dynamic>)['platforms'],
          <String>{'macos'});
      await responses.close();
      await commands.close();
    ...
    testUsingContext('printError should send daemon.logMessage event',
      ...
      final Map<String, dynamic> response =
          await responses.stream.firstWhere((Map<String, dynamic> map) {
        return map['event'] == 'daemon.logMessage' &&
            (map['params'] as Map<String, dynamic>)['level'] == 'error';
      });
      expect(response['id'], isNull);
      expect(response['event'], 'daemon.logMessage');
      final Map<String, String> logMessage =
          castStringKeyedMap(response['params']).cast<String, String>();
    ...
    testUsingContext('should send device.added event when device is discovered',
      ...
      return responses.stream
          ...
          .first
          .then<void>((Map<String, dynamic> response) async {
        expect(response['event'], 'device.added');
        expect(response['params'], isMap);

        final Map<String, dynamic> params =
            castStringKeyedMap(response['params']);
    ...
    testUsingContext('emulator.getEmulators should respond with list',
      ...
      commands
          .add(<String, dynamic>{'id': 0, 'method': 'emulator.getEmulators'});
      final Map<String, dynamic> response =
          await responses.stream.firstWhere(_notEvent);
      expect(response['id'], 0);
      expect(response['result'], isList);
    ...
    testUsingContext('daemon can send exposeUrl requests to the client',
      ...
      final StreamController<Map<String, dynamic>> input =
          StreamController<Map<String, dynamic>>();
      final StreamController<Map<String, dynamic>> output =
          StreamController<Map<String, dynamic>>();
      ...
      unawaited(output.stream
          .firstWhere((Map<String, dynamic> request) =>
              request['method'] == 'app.exposeUrl')
          .then((Map<String, dynamic> request) {
        expect((request['params'] as Map<String, dynamic>)['url'],
            equals(originalUrl));
    ...
    testUsingContext(
      ...
      final Map<String, dynamic> response = await responses.stream
          .firstWhere((Map<String, dynamic> response) => response['id'] == 0);
  ...
  testUsingContext('notifyingLogger outputs trace messages in verbose mode',
      () async {
  ...
  testUsingContext('notifyingLogger ignores trace messages in non-verbose mode',
      () async {
    final NotifyingLogger logger =
  ...
  testUsingContext(
      'notifyingLogger buffers messages sent before a subscription', () async {
    final NotifyingLogger logger =
        NotifyingLogger(verbose: false, parent: bufferLogger);
...
bool _isConnectedEvent(Map<String, dynamic> map) =>
    map['event'] == 'daemon.connected';
