...
Future<ProcessTestResult> runFlutter(
  ...
  Duration expectedMaxDuration = const Duration(
      seconds: 25), // must be less than test timeout of 30 seconds!
}) async {
  ...
  void describeStatus() {
    if (transitions.isNotEmpty) {
      ...
      for (int index = 0; index < transitions.length; index += 1) {
        print('${index.toString().padLeft(5)} '
  ...
  unawaited(process.exitCode.timeout(expectedMaxDuration, onTimeout: () {
    ...
    /* ignore the error here, it'll be reported on the next line */
  }));
...
void main() {
  ...
  testWithoutContext('flutter run help output', () async {
    ...
    expect(result.stdout, <Object>[
      ...
      skip: Platform
          .isWindows); // TODO(jonahwilliams): Re-enable when this test is reliable on device lab, https://github.com/flutter/flutter/issues/81556
}
