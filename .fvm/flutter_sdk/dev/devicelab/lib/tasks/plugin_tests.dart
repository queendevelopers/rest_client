...
class PluginTest {
  PluginTest(this.buildTarget, this.options,
      {this.pluginCreateEnvironment, this.appCreateEnvironment});
  ...
  Future<void> build(String target) async {
    await inDirectory(Directory(rootPath), () async {
      final String buildOutput =
          await evalFlutter('build', options: <String>[target, '-v']);
      ...
      if (target == 'ios' || target == 'macos') {
        ...
        if (buildOutput
            .contains('the range of supported deployment target versions')) {
        ...
        if (target == 'ios') {
          ...
          final int iosDeploymentTargetCount = 'IPHONEOS_DEPLOYMENT_TARGET'
              .allMatches(podsProjectContent)
              .length;
          if (iosDeploymentTargetCount != 9) {
            throw TaskResult.failure(
                'plugintest may contain IPHONEOS_DEPLOYMENT_TARGET, $iosDeploymentTargetCount found');
          }
        ...
        final int macosDeploymentTargetCount =
            'MACOSX_DEPLOYMENT_TARGET'.allMatches(podsProjectContent).length;
        if (target == 'macos' && macosDeploymentTargetCount != 12) {
          throw TaskResult.failure(
              'plugintest may contain MACOSX_DEPLOYMENT_TARGET, $macosDeploymentTargetCount found');
        }
      }
    });
  }
