  ...
  await task(() async {
    ...
    try {
      ...
      for (final String runtime in LineSplitter.split(availableRuntimes)) {
        ...
        final RegExpMatch? iOSRuntimeMatch =
            iOSRuntimePattern.firstMatch(runtime);
        if (iOSRuntimeMatch != null) {
          iOSSimRuntime = iOSRuntimeMatch.group(1)!.trim();
          continue;
        }
        final RegExpMatch? watchOSRuntimeMatch =
            watchOSRuntimePattern.firstMatch(runtime);
      ...
      if (iOSSimRuntime == null || watchSimRuntime == null) {
        ...
        if (iOSSimRuntime != null) {
          message =
              'Found "$iOSSimRuntime", but no watchOS simulator runtime found.';
        } else if (watchSimRuntime != null) {
          message =
              'Found "$watchSimRuntime", but no iOS simulator runtime found.';
        } else {
          message = 'watchOS and iOS simulator runtimes not found.';
        }
        return TaskResult.failure(
            '$message Available runtimes:\n$availableRuntimes');
      }
