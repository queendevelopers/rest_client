...
Future<void> _runABTest() async {
  ...
  if (_taskNames.length > 1) {
    stderr.writeln(
        'When running in A/B test mode exactly one task must be passed but got ${_taskNames.join(', ')}.\n');
    stderr.writeln(_argParser.usage);
    exit(1);
  }
  ...
  if (!args.wasParsed('local-engine')) {
    stderr
        .writeln('When running in A/B test mode --local-engine is required.\n');
    stderr.writeln(_argParser.usage);
    exit(1);
  }
  ...
  final File jsonFile =
      _uniqueFile(args['ab-result-file'] as String? ?? 'ABresults#.json');
      ...
      for (final String nameOrPath in value) {
        ...
        if (fragments.length == 1 && !isDartFile) {
          ...
          _taskNames.add(nameOrPath);
            ...
            !path.equals(path.dirname(nameOrPath), path.join('bin', 'tasks'))) {
