...
final String dart = path.join(flutterRoot, 'bin', 'cache', 'dart-sdk', 'bin',
    Platform.isWindows ? 'dart.exe' : 'dart');
final String pub = path.join(flutterRoot, 'bin', 'cache', 'dart-sdk', 'bin',
...
Future<void> verifyNoMissingLicense(String workingDirectory,
  ...
  await _verifyNoMissingLicenseForExtension(workingDirectory, 'bat',
      overrideMinimumMatches ?? 1, '@ECHO off\n${_generateLicense('REM ')}');
...
Future<void> _verifyNoMissingLicenseForExtension(String workingDirectory,
  ...
  await for (final File file in _allFiles(workingDirectory, extension,
    ...
    if (!contents.startsWith(licensePattern)) errors.add(file.path);
  }
  // Fail if any errors
  if (errors.isNotEmpty) {
...
Future<void> verifyNoBinaries(String workingDirectory,
  ...
  if (!Platform.isWindows) {
    ...
    final List<File> files =
        await _gitFiles(workingDirectory, runSilently: false);
    final List<String> problems = <String>[];
    for (final File file in files) {
...
Future<void> _checkConsumerDependencies() async {
  ...
  final List<String> dependencies = dependencySet.toList()..sort();
  ...
  final ByteConversionSink hasher =
      sha256.startChunkedConversion(controller.sink);
