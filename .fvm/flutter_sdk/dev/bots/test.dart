...
final String pub =
    path.join(flutterRoot, 'bin', 'cache', 'dart-sdk', 'bin', 'pub$bat');
...
Future<void> _runIntegrationToolTests() async {
  ...
  await runCommand(
    ...
    <String>[
      ...
      'activate',
      'devtools',
      File(path.join(flutterRoot, 'bin', 'internal', 'devtools.version'))
          .readAsStringSync(),
    ],
...
Future<void> _runExampleProjectBuildTests(
  ...
  final List<String> additionalArgs =
      hasNullSafety ? <String>['--no-sound-null-safety'] : <String>[];
...
Future<void> _runFrameworkTests() async {
  ...
  Future<void> runMisc() async {
    ...
    await _pubRunTest(path.join(flutterRoot, 'dev', 'conductor'),
        forceSingleCore: true);
...
Future<void> _runFlutterWebTest(
    String workingDirectory, List<String> tests) async {
  await runCommand(
    flutter,
    <String>[
      'test',
...
Future<void> _pubRunTest(
  ...
  if (ensurePrecompiledTool) {
    ...
    // the tool themselves.
    await runCommand(flutter, <String>['--version'],
        environment: pubEnvironment);
  }
