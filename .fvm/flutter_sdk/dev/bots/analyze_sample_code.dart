...
class SampleChecker {
  ...
  String get _snippetsExecutable {
    final String platformScriptPath =
        path.dirname(path.fromUri(Platform.script));
    return path.canonicalize(
        path.join(platformScriptPath, '..', 'snippets', 'lib', 'main.dart'));
  }
  ...
  int checkSamples() {
    ...
    try {
      ...
      _extractSamples(filesToAnalyze,
  ...
  File _writeSample(Sample sample) {
    ...
    final File inputFile = File(path.join(_tempDirectory.path, inputName))
    ...
    if (verbose) stderr.write('${process.stderr}');
...
Future<void> _runInteractive({
  ...
  void analyze(SampleChecker checker, File file) {
    final Map<String, Section> sections = <String, Section>{};
    final Map<String, Sample> snippets = <String, Sample>{};
    checker._extractSamples(<File>[file],
        silent: true, sectionMap: sections, sampleMap: snippets);
    final Map<String, List<AnalysisError>> errors = checker
