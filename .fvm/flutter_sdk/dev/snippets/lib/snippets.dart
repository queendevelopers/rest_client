...
class SnippetGenerator {
  SnippetGenerator({Configuration? configuration})
      : configuration = configuration ??
            ...
            Configuration(
                flutterRoot: Directory(Platform.environment['FLUTTER_ROOT'] ??
  ...
  /// [SnippetType.sample] snippets.
  String _interpolateTemplate(List<_ComponentTuple> injections, String template,
      Map<String, Object?> metadata) {
    final RegExp moustacheRegExp = RegExp('{{([^}]+)}}');
    final String interpolated =
        template.replaceAllMapped(moustacheRegExp, (Match match) {
  ...
  String _interpolateSkeleton(
    ...
    final String channel =
        metadata['channel'] == 'stable' ? 'stable' : 'master';
    ...
    if (type == SnippetType.sample) {
      substitutions
        ...
        ..['app'] = htmlEscape.convert(injections
            .firstWhere((_ComponentTuple tuple) => tuple.name == 'app')
            .mergedContent);
    }
      ...
      if (match != null) {
        ...
        if (match.namedGroup('language') != null) {
          ...
          if (match.namedGroup('section') != null) {
            components.add(_ComponentTuple(
                'code-${match.namedGroup('section')}', <String>[],
                language: language));
          } else {
            components
                .add(_ComponentTuple('code', <String>[], language: language));
          }
        } else {
          language = null;
        }
  ...
  String generate(
    ...
    final List<_ComponentTuple> snippetData =
        _parseInput(_loadFileAsUtf8(input));
    switch (type) {
      case SnippetType.sample:
        ...
        String app =
            _interpolateTemplate(snippetData, templateContents, metadata);
        ...
        final File outputFile =
            output ?? getOutputFile(metadata['id']! as String);
        ...
        final int descriptionIndex = snippetData
            .indexWhere((_ComponentTuple data) => data.name == 'description');
    ...
    final String skeleton = _loadFileAsUtf8(
        configuration.getHtmlSkeletonFile(type, showDartPad: showDartPad));
