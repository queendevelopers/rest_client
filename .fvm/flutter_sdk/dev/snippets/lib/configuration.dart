...
class Configuration {
  ...
  Directory get configDirectory {
    _configPath ??= Directory(path.canonicalize(
        path.join(flutterRoot.absolute.path, 'dev', 'snippets', 'config')));
    return _configPath!;
  }
  ...
  Directory get outputDirectory {
    _docsDirectory ??= Directory(path.canonicalize(path.join(
        flutterRoot.absolute.path, 'dev', 'docs', 'doc', 'snippets')));
  ...
  Directory get skeletonsDirectory =>
      Directory(path.join(configDirectory.path, 'skeletons'));

  /// The directory containing the code templates that can be referenced by the
  /// dartdoc.
  Directory get templatesDirectory =>
      Directory(path.join(configDirectory.path, 'templates'));
