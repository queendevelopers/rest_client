  ...
  group('Generator', () {
    ...
    setUp(() {
      ...
      configuration = Configuration(
          flutterRoot: Directory(path.join(tmpDir.absolute.path, 'flutter')));
...
''');
      configuration
          .getHtmlSkeletonFile(SnippetType.sample)
          .writeAsStringSync('''
<div>HTML Bits</div>
{{description}}
<pre>{{code}}</pre>
...
''');
      configuration
          .getHtmlSkeletonFile(SnippetType.snippet)
          .writeAsStringSync('''
<div>HTML Bits</div>
{{description}}
<pre>{{code}}</pre>
...
''');
      configuration
          .getHtmlSkeletonFile(SnippetType.sample, showDartPad: true)
          .writeAsStringSync('''
...
''');
      ...
      final int materialLine =
          lines.indexOf("import 'package:flutter/material.dart';");
...
''');
      ...
      expect(
          html,
          contains(
              '<iframe class="snippet-dartpad" src="https://dartpad.dev/embed-flutter.html?split=60&run=true&sample_id=id&sample_channel=stable"></iframe>'));
    });
  ...
  group('getChannelName()', () {
    test('does not call git if LUCI_BRANCH env var provided', () {
      ...
      final FakeProcessManager processManager =
          FakeProcessManager(<FakeCommand>[]);
      expect(
        getChannelName(
          platform: platform,
    ...
    test('calls git if LUCI_BRANCH env var is not provided', () {
      ...
      final ProcessResult result =
          ProcessResult(0, 0, '## $branch...refs/heads/master', '');
