...
Error: New codepoints file does not contain all the existing codepoints.\n
    ...
    if (diff > 0) {
      stderr.writeln(
          'It also contains $diff new codepoints: ${newCodepointsSet.difference(oldCodepointsSet)}');
    }
  }
}
...
class _Icon {
  ...
  _Icon(MapEntry<String, String> tokenPair) {
    ...
    for (final MapEntry<String, String> rewritePair
        in identifierRewrites.entries) {
      if (shortId == rewritePair.key) {
        flutterId = id.replaceFirst(
            rewritePair.key, identifierRewrites[rewritePair.key]!);
      }
    }
  ...
  static const List<String> styleSuffixes = <String>[
    '',
    '_outlined',
    '_rounded',
    '_sharp'
  ...
  late String
      name; // e.g. five g, five g outlined, five g rounded, five g sharp
  ...
  String get dartDoc =>
      '<i class="material-icons$htmlSuffix md-36">$shortId</i> &#x2014; material icon named "$name"';
...
''';
  ...
  static String platformAdaptiveDeclaration(
          String fullFlutterId, _Icon agnosticIcon, _Icon iOSIcon) =>
      '''
